...
function srz_fb_gallery_shortcode($atts) {
	...
	if (!$srz_page) return 'Page Not Found';
	...
	$srzfbgallery = new SrizonFBAlbum($srz_page['updatefeed'] * 60, srz_fb_get_access_token($srz_page['app_id'], $srz_page['app_secret']));
	if($set){
		$srz_images_all = $srzfbgallery->get_facebook_albums_images($_REQUEST[$aid],$srz_page['image_sorting']);
		$pagetitle = $srzfbgallery->get_pagetitle($srz_page['pageid'],$set);
	}
	else{
		$srz_images_all = $srzfbgallery->get_facebook_gallery_covers($srz_page['pageid'],$srz_page['album_sorting'], $srz_page['include_exclude'],$srz_page['excludeids']);
	}
